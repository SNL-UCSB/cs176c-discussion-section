...
export NUM_CORES=$3
mkdir -p $OUT_DIR
ls $IN_DIR/*.pcap | parallel -j $NUM_CORES 'echo "ts	sIP	tcp_sPort	udp_sPort	dIP	tcp_dPort	udp_dPort	ip_len	ip_hdr_len	ip_proto	tcp_flags	tcp_seq	tcp_ack	tcp_hdr_len	tcp_rwnd	tls_hostname	dns_hostname	dns_ip" > $OUT_DIR/$(basename {} .pcap).csv && tshark -r {} -T fields -E separator=/t -e frame.time_epoch -e ip.src -e tcp.srcport -e udp.srcport -e ip.dst -e tcp.dstport -e udp.dstport -e ip.len -e ip.hdr_len -e ip.proto -e tcp.flags -e tcp.seq_raw -e tcp.ack_raw -e tcp.hdr_len -e tcp.window_size_value -e tls.handshake.extensions_server_name -e dns.resp.name -e dns.a >> $OUT_DIR/$(basename {} .pcap).csv'
